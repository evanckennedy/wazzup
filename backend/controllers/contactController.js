...
export async function handleCreateContact (req, res) {
  try {
    const token = request.headers.authoriztion.split(' ')[1] // if the authorization header value is "Bearer abc123xyz", the code will split it into an array ["Bearer", "abc123xyz"], and then assign the token "abc123xyz" to the token variable.
